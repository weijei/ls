...
def who_win?(player, dealer)
  ...
  elsif calculate_total(dealer) > 21
...
loop do
  ...
  prompt "Your two cards are #{mycard[0].join} and #{mycard[1].join}"
  ...
  loop do
    ...
    if answer.downcase.start_with?('s') && enough?(mycard)
      prompt "Your points is less than 17. You must hit"
    end
    if !(answer.downcase.start_with?('s') || answer.downcase.start_with?('h'))
      prompt "Your input is invalid"
    end
    break if (answer.downcase.start_with?('s') && !enough?(mycard)) || busted?(mycard)
  end
